...
public class DevicePresenceUpdatesTests(BaseTestEnvironment factory) : BaseSignalRFeatureTest(factory)
{
    public static List<object[]> ValidHomeSharingTypes => HomePriviliges;

    [Fact]
    public async Task UserReceivesDevicePresenceUpdate()
    {
        // Arrange
    ...
    public async Task FriendReceivesDevicePresenceUpdate(string sharingType)
    {
        // Arrange
        var receivedDeviceId = Guid.Empty;
        var (_, connection, brokerId, homeId, deviceId) = await InitializeSignalR();
